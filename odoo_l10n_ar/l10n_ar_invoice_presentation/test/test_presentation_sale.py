...
class TestPresentationSale(TransactionCase):
    ...
    def test_sale_invoice_presentation(self):
        ...
        decoded = base64.decodebytes(self.account_invoice_presentation.sale_file)
        # Fecha de comprobante
        assert decoded[0:8] == b"20000801"
        # Tipo de comprobante
        assert decoded[8:11] == b"001"
        # Punto de venta
        assert decoded[11:16] == b"00001"
        # Numero de comprobante
        assert decoded[16:36] == b"00000000000000000008"
        # Numero de comprobante hasta
        assert decoded[36:56] == b"00000000000000000008"
        # Codigo de documento del comprador
        assert decoded[56:58] == b"80"
        # Numero de identificacion del comprador
        assert decoded[58:78] == b"00000000020307810795"
        # Apellido y nombre del comprador
        assert decoded[78:108] == b"Test Partner                  "
        # Importe total de la operacion
        assert decoded[108:123] == b"000000000121000"
        # Importe total de conceptos que no integran el precio neto gravado
        assert decoded[123:138] == b"000000000000000"
        # Percepcion a no categorizados
        assert decoded[138:153] == b"000000000000000"
        # Importe de operaciones exentas
        assert decoded[153:168] == b"000000000000000"
        # Importe de percepciones o pagos a cuenta de impuestos nacionales
        assert decoded[168:183] == b"000000000000000"
        # Importe de percepciones de ingresos brutos
        assert decoded[183:198] == b"000000000000000"
        # Importe de percepciones impuestos municipales
        assert decoded[198:213] == b"000000000000000"
        # Importe impuestos internos
        assert decoded[213:228] == b"000000000000000"
        # Codigo de moneda
        assert decoded[228:231] == b"PES"
        # Tipo de cambio
        assert decoded[231:241] == b"0001000000"
        # Cantidad de alicuotas de IVA
        assert decoded[241:242] == b"1"
        # Codigo de operacion
        assert decoded[242:243] == b" "
        # Otros tributos
        assert decoded[243:258] == b"000000000000000"
        # Fecha de vencimiento de pago
        assert decoded[258:266] == b"00000000"
