...
class TestInvoice(common.TransactionCase):
    def setUp(self):
        ...
        self.perception = self.env['account.invoice.perception'].create({
            'perception_id': self.env.ref('l10n_ar.1_perception_perception_iibb_pba_sufrida').id,
            'invoice_id': self.invoice.id,
            'amount': 20,
            'base': 2000,
    ...
    def test_multiple_perceptions_values(self):
        self.env['account.invoice.perception'].create({
            'perception_id': self.env.ref('l10n_ar.1_perception_perception_iva_sufrida').id,
            'invoice_id': self.invoice.id,
            'amount': 10,
            'base': 1000,
