...
class TestWizardGeneralLedgerExcel(TransactionCase):
    ...
    def test_values(self):
        ...
        assert list(values.keys()) == ["Asiento ejemplo nombre"]
        vals = values[self.move.name]
        today = date.today().strftime('%d/%m/%Y')
        assert vals[2] == (today, "123456", "Cuenta uno", 1000, '-', "Linea ejemplo 1", "Partner")
        assert vals[1] == (today, "654321", "Cuenta dos", '-', 700, "Linea ejemplo 2", "Partner")
        assert vals[0] == (today, "456789", "Cuenta tres", '-', 300, "Linea ejemplo 3", "Partner")
