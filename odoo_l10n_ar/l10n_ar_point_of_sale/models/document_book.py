...
class DocumentBook(models.Model):
    ...
    def _prev_number(self):
        ...
        return self._get_number(self.name)
    ...
    def _get_number(self, name):
        return '{}-{}'.format(self.pos_ar_id.name.zfill(self.pos_ar_id.prefix_quantity), name.zfill(8))

    def get_number(self, name):
        """
        Devuelve el número formatiado según los parámetros del punto de venta del talonario
        :param name: Número del comprobante (ej 1)
        :return: Número formatiado (ej 0001-00000001)
        """
        return self.sudo()._get_number(name)
