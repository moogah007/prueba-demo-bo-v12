...
class TestAccountRegisterPayments(set_up.SetUp):
    ...
    def test_old_payment_from_wizard(self):
        with self.assertRaises(ValidationError):
            self.payment_wizard.create_payments()

    def test_payment_vals(self):
        payment_vals = self.payment_wizard.get_payments_vals()
        assert payment_vals[0].get('payment_type_line_ids')[0][1] == self.payment_line.id
        assert payment_vals[0].get('pos_ar_id') == self.pos_inbound.id
