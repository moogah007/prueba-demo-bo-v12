...
class TestSoldCheck(common.TransactionCase):
    ...
    def test_move_line_vals_with_commission(self):
        ...
        self.sold_check.commission_account_id = self.env.ref('l10n_ar.1_caja_en_pesos').id
        self.sold_check.interest_account_id = self.env.ref('l10n_ar.1_banco_cuenta_en_pesos').id
        self.sold_check.post()
        lines = self.sold_check.move_id.line_ids
        assert len(lines) == 4
