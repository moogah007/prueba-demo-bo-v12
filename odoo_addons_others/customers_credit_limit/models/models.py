...
class customers_credit_limit(models.Model):
    ...
    def _get_receivable_tax(self):
        ...
        self.env.cr.execute("""SELECT account_move_line.move_id
                      FROM account_move_line
                      LEFT JOIN account_account a ON (account_move_line.account_id=a.id)
		      LEFT JOIN account_account_type act ON (a.user_type_id=act.id)
                      WHERE act.type IN ('receivable')
                      AND account_move_line.partner_id IN %s
                      AND account_move_line.reconciled IS FALSE """ + where_clause + """""", where_params)
        move_ids = self._cr.fetchall()
        if move_ids:
            self.env.cr.execute("""SELECT SUM(account_move_line.credit-account_move_line.debit)
                      FROM account_move_line
                      LEFT JOIN account_account a ON (account_move_line.account_id=a.id)
		      LEFT JOIN account_account_type act ON (a.user_type_id=act.id)
                      ...
                      AND account_move_line.product_id IS NULL
                      AND account_move_line.tax_line_id IS NOT NULL
                      AND account_move_line.move_id IN  %s
                      """ + where_clause + """
                      """,
