...
class TestVATReport(common.TransactionCase):
    ...
    def set_up_invoice(self):
        ...
        self.pos = self.env['pos.ar'].create({'name': 5, 'company_id': self.company.id})
    ...
    def setUp(self):
        ...
        partner = self.env['res.partner'].create({'name': "Partner", 'property_account_position_id': self.iva_ri.id})
        ...
        product = self.env['product.product'].create({'name': "Producto"})
    ...
    def _get_report_lines(self):
        ...
        payment_type = self.env['account.payment.type'].create({'name': "Tipo", 'account_id': self.income_account.id})
        payment_vals = self.cbinvoice._prepare_payment_vals(self.bank_journal, 300, date(date.today().year, date.today().month, 10))
        payment_vals.update({
            'advance_amount': 300,
            'pos_ar_id': self.pos.id,
