...
class TestPayment(common.TransactionCase):
    ...
    def setUp(self):
        ...
        retention_partner_rule = self.env['retention.partner.rule'].create({
            'retention_id': self.env.ref('l10n_ar.1_retention_retention_ganancias_efectuada').id,
    ...
    def test_amount_all_taxed(self):
        invoice = self.env['account.invoice'].new({
            ...
            'denomination_id': self.env['account.denomination'].new({'vat_discriminated': True})
        })
    ...
    def test_amount_untaxed_not_discriminated(self):
        invoice = self.env['account.invoice'].new({
            'amount_total_signed': 500,
            'amount_to_tax': 0,
        })
    ...
    def test_retention_vals(self):
        profit_ret = self.env.ref('l10n_ar.1_retention_retention_ganancias_efectuada')
        vals = self.payment._create_retention(10000, 200, profit_ret)
        assert vals.get('base') == 10000
        assert vals.get('amount') == 200
        assert vals.get('retention_id') == profit_ret.id
        assert vals.get('name') == profit_ret.name
        assert vals.get('jurisdiction') == profit_ret.jurisdiction
        activity = profit_ret.get_profit_retention_rule(self.partner).activity_id
        assert vals.get('activity_id') == activity.id
