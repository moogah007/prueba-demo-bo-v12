...
class TestCashFlow(TransactionCase):
    ...
    def create_invoices(self):
        ...
        self.env['account.invoice.line'].create({
            'name': 'Producto',
            'account_id': self.env.ref('l10n_ar.1_maquinaria_y_equipos').id,
            'quantity': 1,
            'price_unit': 500,
    ...
    def test_search_move_line(self):
        ...
        config = self.create_configuration_empty()
        config.write({
            'account_ids': [(6, 0, [self.account_credit.id, self.account_debit.id])]
        })
        self.create_move()
    ...
    def test_set_invoices(self):
        ...
        invoices_vals = config._set_invoice_values(date.today(), invoices, cash.id)
    ...
    def test_set_invoices_ndd(self):
        ...
        invoices_vals_2 = config._set_invoice_values(date.today(), invoices_2, cash.id)
    ...
    def test_get_check(self):
        ...
        checks_ok = config._get_check_values(today, (today - relativedelta(days=1)), checks, cash.id)
        assert len(checks_ok) == 0
        checks_ok = config._get_check_values(today, today, checks, cash.id)
